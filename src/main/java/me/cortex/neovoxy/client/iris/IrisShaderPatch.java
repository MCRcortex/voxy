...
public class IrisShaderPatch {
    ...
    public static final boolean IMPERSONATE_DISTANT_HORIZONS = System.getProperty("neovoxy.impersonateDHShader", "false").equalsIgnoreCase("true");
    ...
    public static IrisShaderPatch makePatch(ShaderPack ipack, AbsolutePackPath directory, Function<AbsolutePackPath, String> sourceProvider) {
        String neovoxyPatchData = sourceProvider.apply(directory.resolve("neovoxy.json"));
        if (neovoxyPatchData == null) {//No neovoxy patch data in shaderpack
            return null;
        }

        //A more graceful exit on blank string
        if (neovoxyPatchData.isBlank()) {
            return null;
        }

        //Escape things
        neovoxyPatchData = neovoxyPatchData.replace("\\", "\\\\");
        ...
        try {
            ...
            {
                StringBuilder builder = new StringBuilder(neovoxyPatchData.length());
                //Rebuild the patch, replacing commented out " with \"
                for (var line : neovoxyPatchData.split("\n")) {
                ...
                neovoxyPatchData = builder.toString();
            }
            patchData = GSON.fromJson(neovoxyPatchData, PatchGson.class);
            if (patchData == null) {
                throw new IllegalStateException("NeoVoxy patch json returned null, this is most likely due to malformed json file");
            }

            {//Inject data from the auxilery files if they are present
                var opaque = sourceProvider.apply(directory.resolve("neovoxy_opaque.glsl"));
                if (opaque != null) {
                    Logger.info("External opaque shader patch applied");
                    patchData.opaquePatchData = opaque;
                }
                var translucent = sourceProvider.apply(directory.resolve("neovoxy_translucent.glsl"));
                if (translucent != null) {
                ...
                var taa = sourceProvider.apply(directory.resolve("neovoxy_taa.glsl"));
                if (taa != null) {
            ...
            if (invalidPatchDataReason!=null) {
                throw new IllegalStateException("neovoxy json patch not valid: " + invalidPatchDataReason);
            }
        } catch (Exception e) {
            patchData = null;
        ...
        if (patchData.version != VERSION) {
            Logger.error("Shader has neovoxy patch data, but patch version is incorrect. expected " + VERSION + " got "+patchData.version);
