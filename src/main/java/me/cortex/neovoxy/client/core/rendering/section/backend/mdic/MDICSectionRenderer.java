...
import me.cortex.neovoxy.client.core.gl.shader.ShaderLoader;
...
import me.cortex.neovoxy.client.core.rendering.util.LightMapHelper;
...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    ...
    private final Shader commandGenShader = Shader.make()
            ...
            .add(ShaderType.COMPUTE, "neovoxy:lod/gl46/cmdgen.comp")
            .compile();

    private final Shader prepShader = Shader.make()
            .add(ShaderType.COMPUTE, "neovoxy:lod/gl46/prep.comp")
            .compile();

    private final Shader cullShader = Shader.make()
            .add(ShaderType.VERTEX, "neovoxy:lod/gl46/cull/raster.vert")
            .add(ShaderType.FRAGMENT, "neovoxy:lod/gl46/cull/raster.frag")
            .compile();

    private final Shader prefixSumShader = Shader.make()
            //Use subgroup prefix sum if possible otherwise use dodgy... slow prefix sum
            .add(ShaderType.COMPUTE, Capabilities.INSTANCE.subgroup?"neovoxy:util/prefixsum/inital3.comp":"neovoxy:util/prefixsum/simple.comp")
            .define("IO_BUFFER", 0)
            .compile();

    private final Shader translucentGenShader = Shader.make()
            .add(ShaderType.COMPUTE, "neovoxy:lod/gl46/buildtranslucents.comp")
    ...
    public MDICSectionRenderer(AbstractRenderPipeline pipeline, ModelStore modelStore, BasicSectionGeometryData geometryData) {
        ...
        String vertex = ShaderLoader.parse("neovoxy:lod/gl46/quads3.vert");
        ...
        String frag = ShaderLoader.parse("neovoxy:lod/gl46/quads.frag");
    ...
    private void uploadUniformBuffer(MDICViewport viewport) {
        ...
        int earthCurveRatio = NeoVoxyConfig.CONFIG.earthCurveRatio;
    ...
    private void renderTerrain(MDICViewport viewport, long indirectOffset, long drawCountOffset, int maxDrawCount) {
        ...
        if (NeoVoxyClient.getOcclusionDebugState()==3) {
