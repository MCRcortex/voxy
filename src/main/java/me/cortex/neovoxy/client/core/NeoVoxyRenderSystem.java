...
import me.cortex.neovoxy.client.TimingStatistics;
...
import me.cortex.neovoxy.client.core.rendering.ChunkBoundRenderer;
import me.cortex.neovoxy.client.core.rendering.RenderDistanceTracker;
import me.cortex.neovoxy.client.core.rendering.Viewport;
import me.cortex.neovoxy.client.core.rendering.ViewportSelector;
import me.cortex.neovoxy.client.core.rendering.building.RenderGenerationService;
...
import me.cortex.neovoxy.client.core.rendering.util.DownloadStream;
import me.cortex.neovoxy.client.core.rendering.util.PrintfDebugUtil;
import me.cortex.neovoxy.client.core.rendering.util.UploadStream;
import me.cortex.neovoxy.client.core.util.GPUTiming;
...
public class NeoVoxyRenderSystem {
    ...
    public NeoVoxyRenderSystem(WorldEngine world, ServiceManager sm) {
        ...
        try {
...

            {
                ...
                if (NeoVoxyCommon.IS_MINE_IN_ABYSS) {//TODO: make this somehow configurable
                    minSec = -8;
                    maxSec = 7;
                }
                ...
                this.setRenderDistance(NeoVoxyConfig.CONFIG.sectionRenderDistance);
            }

            this.chunkBoundRenderer = new ChunkBoundRenderer(this.pipeline);

            Logger.info("NeoVoxy render system created with " + geometryCapacity + " geometry capacity, using pipeline '" + this.pipeline.getClass().getSimpleName() + "' with renderer '" + sectionRenderer.getClass().getSimpleName() + "'");
    ...
    public Viewport<?> setupViewport(ChunkRenderMatrices matrices, double cameraX, double cameraY, double cameraZ) {
        ...
        if (NeoVoxyClient.getOcclusionDebugState()==0) {
            viewport.frameId++;
        }
    ...
    public void renderOpaque(Viewport<?> viewport) {
        ...
        // MC 1.21.1 NeoForge: Fog is handled by NeoVoxyClientEvents.onRenderFog()
        ...
        if ((!NeoVoxyClient.disableSodiumChunkRender())&&!false) {
        ...
        //As much dynamic runtime stuff here
        {
            ...
            while (this.renderDistanceTracker.setCenterAndProcess(viewport.cameraX, viewport.cameraZ) && NeoVoxyClient.isFrexActive());//While FF is active, run until everything is processed
            TimingStatistics.H.start();
            //Done here as is allows less gl state resetup
            do { this.modelService.tick(900_000); } while (NeoVoxyClient.isFrexActive() && !this.modelService.areQueuesEmpty());
    ...
    private void autoBalanceSubDivSize() {
        ...
        if (Minecraft.getInstance().getFps() < MIN_FPS) {
            NeoVoxyConfig.CONFIG.subDivisionSize = Math.min(NeoVoxyConfig.CONFIG.subDivisionSize + INCREASE_PER_SECOND / Math.max(1f, Minecraft.getInstance().getFps()), 256);
        }

        if (MAX_FPS < Minecraft.getInstance().getFps() && canDecreaseSize) {
            NeoVoxyConfig.CONFIG.subDivisionSize = Math.max(NeoVoxyConfig.CONFIG.subDivisionSize - DECREASE_PER_SECOND / Math.max(1f, Minecraft.getInstance().getFps()), 28);
        }
    }
    ...
    private static Matrix4f computeProjectionMat(Matrix4fc base) {
        ...
        // at short render distances the vanilla terrain doesnt end up covering the 16f near plane neovoxy uses
        ...
        float nearNeoVoxy = Minecraft.getInstance().gameRenderer.getRenderDistance()<=32.0f?8f:16f;
        nearNeoVoxy = NeoVoxyClient.disableSodiumChunkRender()?0.1f:nearNeoVoxy;

        return base.mulLocal(
                makeProjectionMatrix(0.05f, Minecraft.getInstance().gameRenderer.getDepthFar()).invert(),
                new Matrix4f()
        ).mulLocal(makeProjectionMatrix(nearNeoVoxy, 16*3000));
    }

    private boolean frexStillHasWork() {
        if (!NeoVoxyClient.isFrexActive()) {
            return false;
        }
    ...
    public void addDebugInfo(List<String> debug) {
        ...
        }
        {
            ...
            debug.add("NeoVoxy frame runtime (millis): " + TimingStatistics.dynamic.pVal() + ", " + TimingStatistics.main.pVal()+ ", " + TimingStatistics.postDynamic.pVal()+ ", " + TimingStatistics.all.pVal());
    ...
    private static long getGeometryBufferSize() {
        ...
        var override = System.getProperty("neovoxy.geometryBufferSizeOverrideMB", "");
        if (!override.isEmpty()) {
