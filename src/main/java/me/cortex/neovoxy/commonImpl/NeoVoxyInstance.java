...
import me.cortex.neovoxy.common.thread.UnifiedServiceThreadPool;
import me.cortex.neovoxy.common.util.MemoryBuffer;
import me.cortex.neovoxy.common.world.WorldEngine;
import me.cortex.neovoxy.common.world.service.SectionSavingService;
...
public abstract class NeoVoxyInstance {
    ...
    public NeoVoxyInstance() {
        Logger.info("Initializing neovoxy instance");
    ...
    protected void setNumThreads(int threads) {
        ...
        if (this.threadPool.setNumThreads(threads)) {
            Logger.info("Dedicated neovoxy thread pool size: " + threads);
        }
    }
    ...
    public WorldEngine getOrCreate(WorldIdentifier identifier, boolean incrementRef) {
        if (!this.isRunning) {
            Logger.error("Tried getting world object on neovoxy instance but its not running");
            return null;
        }
    ...
    public void shutdown() {
        Logger.info("Shutting down neovoxy instance");
